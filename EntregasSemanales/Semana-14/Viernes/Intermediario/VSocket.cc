...
/**
  *  SetSockOpt
  * Se agrega este método para mantener todo en vsocket
  * Sirve para hacer el broadcast
  * 
 **/
int VSocket::SetSockOpt( int level, int optname, const void * optval, socklen_t optlen ) {
   int st = setsockopt( this->sockId, level, optname, optval, optlen );
   if ( -1 == st ) {
      throw std::runtime_error( "VSocket::SetSockOpt" );
   }
   return st;
}
