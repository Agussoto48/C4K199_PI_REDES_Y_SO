...
{ // System call 6
   ...
   machine->WriteRegister(2, leidos);
}
...
{ // System call 7
   ...
   {
      hechos = write(file, buffer, size);
   }
   machine->WriteRegister(2, hechos);
}
/*
...
void NachOS_Socket()
{ // System call 30
   int familia = machine->ReadRegister(4);
   int type = machine->ReadRegister(5);

   int file = socket(familia, type, 0);

   machine->WriteRegister(2, file);
}
static void ReadUserString(int addr, char *target, int max)
{
   int value = 0;
   int i = 0;
...
{ // System call 31
   int file = machine->ReadRegister(4);
   int ip_addr = machine->ReadRegister(5);
   int port = machine->ReadRegister(6);

   char ip[64];

   ReadUserString(ip_addr, ip, 64);
   ...
   int resultado = connect(file, (struct sockaddr *)&servidor, sizeof(servidor));
